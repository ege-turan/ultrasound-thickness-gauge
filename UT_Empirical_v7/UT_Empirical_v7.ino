...
// WORKS WORSE THAN BEFORE 8/26/25
...
#define REF_ALPHA         0.05    // dynamic reference update rate
...
void loop() {
  for (int j = 0; j < 18; j++) {
    ...
    // -------- Dynamic reference subtraction --------
    if (referenceSet) {
      for (int i = 0; i < MAX_SAMPLES; i++) {
        reference[i] = (1 - REF_ALPHA) * reference[i] + REF_ALPHA * values[i];
    ...
    // -------- Serial CSV logging --------
    if (DEBUG_MODE && digitalRead(pin_input_reference) == HIGH) {
